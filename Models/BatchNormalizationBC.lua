...
local BatchNormalizationBC,parent = torch.class('BatchNormalizationBC', 'nn.Module')

function BatchNormalizationBC:__init(nOutput, eps, momentum)
   parent.__init(self)
   ...
   self.train = true
   self.momentum = momentum or 0.15
...
function BatchNormalizationBC:reset()
   self.weight:fill(1)
   self.bias:zero()
end

function BatchNormalizationBC:updateOutput(input)
   ...
   if self.train == false then
    ...
    --  print(self.std:size(),input:norm(1,1):size())
      --self.std:copy(self.centered:norm(1,1):div(self.centered:size(1)):add(1e-5):pow(-1))
    --  print(self.std:size())
...
function BatchNormalizationBC:updateGradInput(input, gradOutput)
...
function BatchNormalizationBC:accGradParameters(input, gradOutput, scale)
