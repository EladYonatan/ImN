...
function cudnnBinarySpatialConvolution:__init(nInputPlane, nOutputPlane,
    ...
    self.convmodel:add(nn.SpatialConvolution(1,1,3,3,1,1))
...
function cudnnBinarySpatialConvolution:binarized(trainFlag)
  ...
  local numMap=self.weight:size(1)
  local l1=torch.norm(self.weight:view(numMap,-1),1,2)
  --print(l1:size())
  l1=l1:view(numMap,1,1,1):expandAs(self.weight)
  ...
  self.weightB:cmul(l1:div(self.weight:size(1)))
