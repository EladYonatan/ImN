...
cmd:option('-LR',                 1e-6,                     'learning rate')
cmd:option('-LRDecay',            1e-10,                        'learning rate decay (in # samples)')
...
local function Forward(DB, train)
    ...
    while NumSamples < SizeData do
        ...
        while MiniBatch:getNextBatch() do
            ...
            --print(currLoss)
            res,ind=y:topk(5,true)
            qqq=torch.expandAs(yt:view(ind:size(1),1),ind)
          --  qqq=qqq:long()
          --  print(ind:type(),qqq:type())
           --print(top5Tot)
            top5Curr=ind:eq(qqq):sum()
            lossVal = currLoss + lossVal
            top5Tot=top5Tot+top5Curr;
        ...
        if train and opt.checkpoint >0 and (currBatch % math.ceil(opt.checkpoint/opt.bufferSize) == 0) then
            ...
            print('\nAfter ' .. NumSamples .. ' samples, current top5 is: ' .. top5Tot/NumSamples .. '\n')
            torch.save(netFilename .. '_checkpoint' .. '.t7', savedModel)
        end
        collectgarbage()
    end
    print('\nThe top 5 res is: ' .. top5Tot/SizeData .. '\n')
    xlua.progress(NumSamples, SizeData)
...
while epoch ~= opt.epoch do
    ...
    optimState.learningRate= optimState.learningRate*0.99
    if epoch%20==0  then
      optimState.learningRate=optimState.learningRate*0.25
    end
