...
cmd:option('-LRDecay',            1e-5,                        'learning rate decay (in # samples)')
...
local function Forward(DB, train)
    ...
    while NumSamples < SizeData do
        ...
        while MiniBatch:getNextBatch() do
            ...
            if train then
                y, currLoss = optimizer:optimize(x, one_hot_yt)
                if opt.nGPU > 1 then
            ...
            --print(one_hot_yt[1][yt[1]])
            print(y[1][yt[1]],y[1]:mean())
            --print(yt[1])
            tmp=one_hot_yt-y
--            print(tmp:size())
            aaa=torch.sqrt(tmp:pow(2):sum(2):mean())
            print(aaa)
